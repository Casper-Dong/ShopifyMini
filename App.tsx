...
export function App() {
  ...
  // --- Purchase & Discount Summary from Orders ---
  ...
  // --- Random Shapes for Vendor Section ---
  const vendorShapes = useMemo(() => {
    ...
    const count = Math.floor(Math.random() * 2) + 4 // 4-5 shapes for neatness
    for (let i = 0; i < count; i++) {
      const type = shapeTypes[Math.floor(Math.random() * shapeTypes.length)]
      const size = Math.floor(Math.random() * 24) + 24 // 24-48px
      const top = Math.floor(Math.random() * 70) // 0-70%
  ...
  return (
    <div className="min-h-screen w-full pt-10 px-2 pb-6 relative overflow-hidden bg-gradient-to-br from-green-400 via-teal-400 to-blue-500">
      {/* --- Header --- */}
      <div className="relative z-10 max-w-md mx-auto">
        <h1 className="text-2xl font-bold mb-2 text-center drop-shadow-lg">Welcome to Shop Minis!</h1>
        <p className="text-xs text-blue-600 mb-4 text-center bg-blue-50 py-2 px-4 rounded border border-blue-200 shadow-sm">
          🛠️ Edit <b>src/App.tsx</b> to change this screen and come back to see your edits!
        </p>
        {/* --- Frequently Bought Vendors Section --- */}
        ...
        <div className="relative min-h-[180px] mb-6">
          ...
          <div className="relative z-10 flex flex-col gap-3">
            {vendorStats.map((vendor) => (
              <div key={vendor.vendor} className="bg-white/90 rounded-lg shadow-md p-4 flex flex-col items-center border border-gray-100">
        ...
        <div className="mb-8 bg-green-50/90 border border-green-200 rounded-lg p-4 flex flex-col items-center shadow-md">
        ...
        <p className="text-base text-gray-600 mb-6 text-center">These are the popular products today</p>
